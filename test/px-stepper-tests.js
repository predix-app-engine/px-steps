...
function runBaseTests() {
  var pathGuideFixture = fixture('test_fixture');
  path_guide = document.getElementById('path_guide');

  suite('Base Automation Tests for px-path-guide', function() {

    test('Polymer exists', function() {
      assert.isTrue(Polymer !== null);
    });
    test('px-path-guide fixture is created', function() {
      assert.isTrue(document.getElementById('path_guide') !== null);
    });
    test('There should be 4 steps', function() {
      var steps = Polymer.dom(pathGuideFixture.root).querySelectorAll('div.step-connector');
      assert(steps.length === 4, 'there is not exactly 4 steps');
    });
  });
}
